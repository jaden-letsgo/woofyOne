...
def getname(links):
    ...
    for link in links:
        ...
        dogname = soup.find('body').find('div', class_='cmw bgc-white page-single-breed').find('div', class_='page-container').find('ul', class_='breadcrumbs show-at-medium').find('li', class_='breadcrumbs__item is-active').text
        name.append(dogname)
    print(len(name))
    return name


def make_connect(db_file):
    conn = None
    try:
...
def insert(cursor, item):
    ...
    query = """INSERT or REPLACE INTO DogTable
            (DogName)
            VALUES (?);"""
    data = ((item,))
...
links = ['https://www.akc.org/dog-breeds/affenpinscher/',
